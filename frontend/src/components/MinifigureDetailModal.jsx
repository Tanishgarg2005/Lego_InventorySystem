...
function MinifigureDetailModal({ minifigure, onClose, onRefresh, onEdit }) {
  ...
  const handleDelete = () => {
    if (
      window.confirm(
        `Are you sure you want to delete ${minifigure.name}? This cannot be undone.`
      )
    ) {
      axios
        .delete(`http://localhost:8080/api/minifigures/${minifigure.id}`)
        .then(() => {
          onRefresh();
          onClose();
        })
        .catch((error) =>
          console.error("There was an error deleting the minifigure!", error)
        );
    }
  };
