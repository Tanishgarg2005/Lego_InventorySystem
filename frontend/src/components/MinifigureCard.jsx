// frontend/src/components/MinifigureCard.jsx
...
function MinifigureCard({ minifigure, onSelect }) {
  return (
    ...
    >
      ...
      <div className="p-4">
        <h3 className="font-bold text-base text-slate-800 truncate" title={minifigure.name}>{minifigure.name}</h3>
        <p className="text-sm text-gray-600 truncate">{minifigure.theme.name}</p>
        <p className="mt-2 font-semibold text-base text-slate-700">${minifigure.estimatedValue || '0.00'}</p>
      </div>
    </div>
  );
}
